...
class CamelotPDFFormatter(HeuristicPDFFormatter):
    ...
    def process_tables_and_images(self, datariot_result):
        ...
        for idx, bbox in enumerate(datariot_result.bboxes):
            if isinstance(bbox, PDFTableBox):
                ...
                if len(tables) == 0:  # no table found either with lattice nor stream
                    ...
                    images_within_table = [box for box in datariot_result.bboxes if (isinstance(box, PDFImageBox) and (
                            box.page_number == bbox.page_number) and self.first_includes_second(table_bbox, box,
                                                                                                page_height))]
        ...
        for page_to_process in all_page_numbers:
            ...
            page_box_dummy = PDFTextBox(0, 0, page_width, page_height, "page", font_size=0, font_name="",
                                        page_number=0)
            insert_dict = {}
            bg_img_list = []
    ...
    def process(self):
        ...
        if num_overall_headers > 0:
            for hlevel in range(1, max(header_counts.keys()) + 1):
        ...
        for split in pdf_splits_recursive:
            ...
            if split.data["page"] > running_page_content:  # save previous page, start new page
                ...
                header_text = (split.data["Title"] if "Title" in split.data.keys() else "") + "".join(
                    [f" / {x}" for x in
                     [split.data[f"Header {k}"] for k in range(0, 10) if f"Header {k}" in split.data.keys()]])
